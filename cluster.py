...
def gen_clustered_image(file_path, analyses, num_aggres, bands, num_clusters=3, sub_image_size=256, sub_image_start=[0, 0], map_width_to_meters=1.0, map_height_to_meters=1.0):
    ...
    with rasterio.open(file_path) as img:
        ...
        sub_img = np.empty((img.count, sub_image_size, sub_image_size))
        size = sub_image_size
    ...
    try:
        ...
        for band in np.unique(bands):
            ...
            for num_aggre in range(maxNumAggre):
                ...
                dem_data.export(exportPath(file_path, band, num_aggre))
    finally:
    ...
    for index in range(len(analyses)):
        if analyses[index] is Analyses.regression:
            ...
            fractal = rbg.fractal(band, num_aggres[index], 127)
...
analyses = [Analyses.regression, Analyses.regression, Analyses.regression, Analyses.regression]
num_aggres = [3, 4, 5, 6]
bands = [[1, 2], [1, 2], [1, 2], [1, 2]]

gen_clustered_image(test_file_name, analyses=analyses, num_aggres=num_aggres, bands=bands, num_clusters=6, sub_image_size=512)