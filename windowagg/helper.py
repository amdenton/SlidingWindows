...
def plot(file_name):
    with rasterio.open(file_name) as img:
        if img.count > 1:
            data = np.empty([img.shape[0], img.shape[1],
                            img.count]).astype(np.uint8)
...
#            plt.title(file_name)
            ...
            plt.text(0, -shift, 'min: '+str(round(np.amin(data), 3)) +
                     ' max: '+str(round(np.amax(data), 3)))
...
def create_tif(arr_in, file_name, profile=None, num_aggre=0, pixelSpatialResolution=(1, 1), dtype=rasterio.int8):
    if (profile == None):
    ...
    new_transform = Affine.translation(
        img_offset * pixelSpatialResolution[0], img_offset * pixelSpatialResolution[1]) * old_transform
...
def arr_dtype_conversion(arr_in, dtype=np.uint16, low_bound=None, high_bound=None):
    ...
    if (high_bound == low_bound):
        ...
        arr_out = ((arr_in - low_bound) / (high_bound - low_bound)
                   * new_dtype_max).astype(dtype)
