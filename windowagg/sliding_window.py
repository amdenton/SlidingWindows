...
class SlidingWindow:
    ...
    def __init__(self, file_path, cell_width=1, cell_height=1):
        ...
        self.__width_meters = pixel_width * cell_width
        self.__height_meters = pixel_height * cell_height
    ...
    def __create_tif(self, arr_in, agg_window_len=1, is_export=False, file_name=None):
        if (type(arr_in) != list):
            arr_in = [arr_in]
        ...
        if (not is_export):
            ...
            pixel_width = math.sqrt(transform[0]**2 + transform[3]**2)
            pixel_height = math.sqrt(transform[1]**2 + transform[4]**2)
            temp[2] = transform[2] + (agg_window_len - 1) * pixel_width / 2
            temp[5] = transform[5] - (agg_window_len - 1) * pixel_height / 2
            temp[0] = transform[0] * agg_window_len
            temp[1] = transform[1] * agg_window_len
            temp[3] = transform[3] * agg_window_len
            temp[4] = transform[4] * agg_window_len
        ...
        if (file_name == None):
            ...
            file_name = os.path.splitext(self.__file_name)[0] + '_' + caller_name + '.tif'
            
        with rasterio.open(file_name, 'w', **profile, BIGTIFF=big_tiff) as dst:
            if (is_export):
                dst.update_tags(ns='DEM_UTILITIES', aggregation_window_length=str(self.__agg_window_len))
    ...
    def aggregation(self, operation, num_aggre):        
        ...
        return self.__create_tif(arr, agg_window_len=2**num_aggre)
    ...
    def _partial_aggregation(self, arr_in, power_start, power_target, operation):
        ...
        for i in range(power_start, power_target):
            ...
            if operation.upper() == Agg_ops.add_all:
                arr_out = top_left + top_right + bottom_left + bottom_right
            if operation.upper() == Agg_ops.add_bottom:
                arr_out = -top_left - top_right + bottom_left + bottom_right
            if operation.upper() == Agg_ops.add_right:
                arr_out = -top_left + top_right - bottom_left + bottom_right
            if operation.upper() == Agg_ops.add_main_diag:
                arr_out = top_left - top_right - bottom_left + bottom_right
            elif operation.upper() == Agg_ops.maximum:
    ...
    def regression(self, band1, band2, num_aggre):
        ...
        return self.__create_tif(arr_m, agg_window_len=2**num_aggre)
    ...
    def pearson(self, band1, band2, num_aggre):
        ...
        return self.__create_tif(arr_r, agg_window_len=2**num_aggre)
    ...
    def fractal(self, band, threshold, power_start, power_target):
        ...
        return self.__create_tif(arr, agg_window_len=2**power_target)
    ...
    def dem_initialize_arrays(self):
        ...
        self.__agg_window_len = 1
    
    def dem_export_arrays(self):
        agg_window_len = self.__agg_window_len
        export = []
        for key in self.__dem_arr_dict:
            export.append(self.__dem_arr_dict[key])
        file_name = os.path.splitext(self.__file_name)[0] + '_export_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(export, agg_window_len=agg_window_len, is_export=True, file_name=file_name)
    ...
    def dem_import_arrays(self):
        ...
        for key in self.__dem_arr_dict:
            ...
            i += 1
        self.__agg_window_len = int(self.__img.tags(ns='DEM_UTILITIES')['aggregation_window_length'])
    ...
    def dem_aggregation_step(self, num_steps):
        ...
        agg_window_len = self.__agg_window_len
        delta_power = int(math.log2(agg_window_len))

        for _ in range(num_steps):
            agg_window_len *= 2

            z_sum_all = self._partial_aggregation(z, delta_power, (delta_power + 1), Agg_ops.add_all)
            z_sum_bottom = self._partial_aggregation(z, delta_power, (delta_power + 1), Agg_ops.add_bottom)
            z_sum_right = self._partial_aggregation(z, delta_power, (delta_power + 1), Agg_ops.add_right)
            z_sum_main_diag = self._partial_aggregation(z, delta_power, (delta_power + 1), Agg_ops.add_main_diag)

            xz_sum_all = self._partial_aggregation(xz, delta_power, (delta_power + 1), Agg_ops.add_all)
            xz_sum_bottom = self._partial_aggregation(xz, delta_power, (delta_power + 1), Agg_ops.add_bottom)
            xz_sum_right = self._partial_aggregation(xz, delta_power, (delta_power + 1), Agg_ops.add_right)

            yz_sum_all = self._partial_aggregation(yz, delta_power, (delta_power + 1), Agg_ops.add_all)
            yz_sum_bottom = self._partial_aggregation(yz, delta_power, (delta_power + 1), Agg_ops.add_bottom)
            yz_sum_right = self._partial_aggregation(yz, delta_power, (delta_power + 1), Agg_ops.add_right)

            xxz_sum_all = self._partial_aggregation(xxz, delta_power, (delta_power + 1), Agg_ops.add_all)

            yyz_sum_all = self._partial_aggregation(yyz, delta_power, (delta_power + 1), Agg_ops.add_all)

            xyz_sum_all = self._partial_aggregation(xyz, delta_power, (delta_power + 1), Agg_ops.add_all)

            z = 0.25 * z_sum_all
            xz = 0.25 * (xz_sum_all + (0.25 * agg_window_len * z_sum_right))
            yz = 0.25 * (yz_sum_all + (0.25 * agg_window_len * z_sum_bottom))
            xxz = 0.25 * (xxz_sum_all + (.5 * agg_window_len * xz_sum_right) + (0.0625 * agg_window_len**2 * z_sum_all))
            yyz = 0.25 * (yyz_sum_all + (.5 * agg_window_len * yz_sum_bottom) + (0.0625 * agg_window_len**2 * z_sum_all))
            xyz = 0.25 * (xyz_sum_all + (0.25 * agg_window_len * (xz_sum_bottom + yz_sum_right)) + (0.0625 * agg_window_len**2 * z_sum_main_diag))

            delta_power += 1
        
        self.__dem_arr_dict.update({'z': z, 'xz': xz, 'yz': yz, 'xxz': xxz, 'yyz': yyz, 'xyz': xyz})
        self.__agg_window_len = agg_window_len
    ...
    def dem_mean(self, arr_name='z'):
        ...
        arr = _Utilities._arr_dtype_conversion(arr, np.uint16)
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_' + arr_name + '_mean_w' + str(agg_window_len) + '.tif'
        return self.__create_tif(arr, agg_window_len=agg_window_len, file_name=file_name)

    # generate image of aggregated slope values
    def dem_slope(self):
        slope = self.__slope()
        slope = _Utilities._arr_dtype_conversion(slope, np.uint16, low_bound=0, up_bound=np.iinfo(np.uint16).max)
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_slope_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(slope, agg_window_len=agg_window_len, file_name=file_name)
    ...
    def __slope(self):
        ...
        w = self.__width_meters
        h = self.__height_meters
        agg_window_len = self.__agg_window_len
    ...
    def dem_slope_angle(self):
        ...
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_slope_angle_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(slope_angle, agg_window_len=agg_window_len, file_name=file_name)
    ...
    def dem_aspect(self):
        ...
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_aspect_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(aspect, agg_window_len=agg_window_len, file_name=file_name)
    ...
    def dem_profile(self):
        ...
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_profile_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(profile, agg_window_len=agg_window_len, file_name=file_name)
    ...
    def dem_planform(self):
        ...
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_planform_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(planform, agg_window_len=agg_window_len, file_name=file_name)
    ...
    def dem_standard(self):
        ...
        agg_window_len = self.__agg_window_len
        file_name = os.path.splitext(self.__file_name)[0] + '_standard_w' + str(agg_window_len) +'.tif'
        return self.__create_tif(standard, agg_window_len=agg_window_len, file_name=file_name)
